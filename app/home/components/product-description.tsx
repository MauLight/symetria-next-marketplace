...
import { addToCart, getPercentage } from "@/app/functions/functions"
...
export default function ProductDescription({ id, title, discount, price }: DescriptionProps) {
    return (
        <div className={"w-full absolute bottom-5 flex justify-between px-5 z-10 transition-all duration-300"}>
            ...
            <div className="flex items-center gap-x-5">
                ...
                <button onClick={() => { addToCart({ id, title, discount, price }) }} aria-label='add to cart' className='h-[50px] w-[50px] antialiased rounded-full bg-black border-t border-sym_gray-300 shadow-sm shadow-sym_gray-800 bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-70 flex justify-center items-center cursor-pointer text-[#ffffff] hover:text-indigo-500'>
