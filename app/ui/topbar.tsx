...
export default async function Topbar() {
    ...
    return (
        <nav className="fixed top-0 left-0 w-full flex justify-center items-center z-50 max-md:pl-5 max-md:pr-2 md:max-[1440px]:px-5 bg-sym-light dark:bg-black border-b border-sym-border-light dark:border-sym-border">
            <div className='w-full max-w-[1440px] h-[60px] flex justify-between items-center'>
                <Link href={'/home'}>
                    <h1 className='text-[1.1rem] text-sym-text-light-focus dark:text-sym-text-primary'>E-Commerce</h1>
                </Link>
                <div className='hidden md:flex items-center gap-x-8 text-[0.9rem]'>

                    <div className='relative group '>
                        <input className='w-[250px] h-8 border border-sym-border-light dark:border-sym-border text-sym-text-light dark:text-sym-text-primary outline-none pl-12 rounded-[8px]' type="text" />
                        <MagnifyingGlassIcon className='absolute top-[6px] text-gray-400 dark:text-gray-500 group-hover:text-sym-text-primary left-2 w-5 h-5 transition-color duration-300' />
                    </div>
