...
function CustomDropdownWithCreate({
    ...
    return (
        <div ref={dropRef} id={id} onClick={() => { setIsOpen(!isOpen) }} className='relative w-full h-9 flex justify-between items-center rounded-[3px] border border-sym-border-light dark:border-sym-border ring-0 focus:ring-0 focus:outline-none cursor-pointer'>
            ...
            }
            {
                isOpen && (
                    <motion.div
                        ...
                        className='absolute top-9 left-0 z-20 w-full max-h-[200px] overflow-y-scroll border border-sym-border-light dark:border-sym-border rounded-b-[5px] shadow-xl dark:shadow-black'>
                        <>
                            {
                                error ? (
                                                        ...
                                                        <>
                                                            ...
                                                            }
                                                            {
                                                                list.map((item: string, i: number) => (
                                                                    <button key={`${item}-${i}`} onClick={() => { setChoice(item) }} className={`w-full h-9 bg-sym-light dark:bg-black text-sym-text-light dark:text-sym-text-secondary ring-0 focus:ring-0 focus:outline-none px-2 placeholder-[#292929] active:bg-[#10100e] transition-color duration-200`}>
                                                                        {
                                                                            item
                                                                        }
                                                                    </button>
